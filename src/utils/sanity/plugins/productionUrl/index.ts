/**
 * This plugin sets up the "Open preview (CTRL + ALT + O)" in the dropdown menu
 */
...
export const productionUrl = definePlugin<PluginParams>(
    ...
    return {
      ...
      document: {
        productionUrl: async (prev, { document, getClient }) => {
          const url = new URL("/api/content/preview", location.origin);
